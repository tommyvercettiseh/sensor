...
public class SessionOverlay extends Overlay
{
	...
	@Inject
	public SessionOverlay(SensorPlugin plugin, SensorConfig config)
	{
		this.plugin = plugin;
		this.config = config;
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		drawCard(graphics, width, height, loggedIn);
		return new Dimension(width, height);
	}

	private void drawCard(Graphics2D graphics, int width, int height, boolean loggedIn)
	{
		String label = "SESSION";
		String status = loggedIn ? "LOGGED IN" : "LOGGED OUT";
		int pad = Math.max(7, Math.min(14, height / 5));
		int barHeight = Math.max(6, Math.min(16, height / 5));
		int fontSize = Math.max(11, Math.min(22, height / 3));
		...
		graphics.setColor(loggedIn ? GREEN : RED);
